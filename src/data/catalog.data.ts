import type { CatalogEntry } from "../types";

export const CATALOG_DATA: CatalogEntry[] = [
  {
    patternId: "style-switcher",
    name: "Style Switcher",
    description: "Switch basemap styles without losing viewport or overlay context.",
    capabilities: ["setStyle", "style.load", "camera preserve", "overlay restore"],
    category: "layers",
    subcategory: "basemaps",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "style-loader",
    name: "Style JSON / URL Loader",
    description: "Paste a style URL or raw JSON and load it safely with validation.",
    capabilities: ["setStyle", "style URL", "style JSON", "validation", "safe reload"],
    category: "layers",
    subcategory: "basemaps",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "map-compare-swipe",
    name: "Map Compare / Swipe",
    description: "Compare two styles with a swipe divider and synced camera.",
    capabilities: ["two maps", "camera sync", "swipe divider", "setStyle"],
    category: "layers",
    subcategory: "basemaps",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "terrain-exaggeration",
    name: "Terrain Exaggeration Controls",
    description: "Control terrain exaggeration and optional sky/hillshade.",
    capabilities: ["terrain", "raster-dem", "exaggeration", "hillshade", "sky"],
    category: "layers",
    subcategory: "basemaps",
    tags: ["interactive", "3d", "raster"],
    provider: "mapbox",
  },
  {
    patternId: "layer-basics",
    name: "Layer Basics",
    description: "Fundamental Mapbox GL layer types and styling properties.",
    capabilities: ["fill", "line", "circle", "symbol"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "layer-inspector",
    name: "Layer Inspector",
    description:
      "Inspect and query map layers with click and hover interactions.",
    capabilities: ["queryRenderedFeatures", "popups", "hover state"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "layer-explorer",
    name: "Layer Explorer",
    description: "Browse and toggle visibility of all layers in a map style.",
    capabilities: ["layer listing", "visibility toggle", "filter"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "fill-patterns",
    name: "Fill Patterns",
    description:
      "Apply hatching/stripe/dot fill patterns to polygons using addImage + fill-pattern.",
    capabilities: ["addImage", "fill-pattern", "canvas patterns", "legend"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive", "geojson"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
  },
  {
    patternId: "hover-tooltips",
    name: "Hover Tooltips",
    description:
      "Lightweight hover tooltips that follow the cursor without popup churn.",
    capabilities: ["queryRenderedFeatures", "hover", "tooltip", "throttle"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
  },
  {
    patternId: "property-filtering",
    name: "Property-Based Filtering UI",
    description:
      "Filter visible features by property values with a small UI that drives setFilter (dual-engine Mapbox/MapLibre).",
    capabilities: ["setFilter", "expressions", "multi-filter UI", "clearable filters"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive", "geojson"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "clip-simplify",
    name: "Viewport Clip + Simplification",
    description: "Client-side GeoJSON clipping and Douglas-Peucker simplification using Turf.js.",
    capabilities: ["bboxClip", "simplify", "viewport bounds", "before/after stats"],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive", "geojson", "turf"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "geojson-vt",
    name: "Client-Side Tiling (geojson-vt)",
    description: "Render large GeoJSON using client-side tiling to reduce per-frame cost.",
    capabilities: ["geojson-vt", "client-side tiling", "viewport filtering", "raw vs tiled comparison"],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "streaming-updates",
    name: "Streaming Updates (setData)",
    description:
      "Simulate incremental feature updates and keep the map responsive (dual-engine Mapbox/MapLibre).",
    capabilities: ["GeoJSON source", "setData", "incremental updates", "pause/resume"],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive", "geojson", "animation"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "symbol-labels-icons",
    name: "Symbol Labels + Icons",
    description:
      "Symbol layer patterns for labels + icons, including SDF recoloring and collision settings.",
    capabilities: ["symbol", "text", "SDF icons", "halo", "collision"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive", "vector"],
    provider: "mapbox",
  },
  {
    patternId: "line-decorations",
    name: "Line Decorations",
    description:
      "Line styling patterns for routes/paths: dashed, gradient, and directional arrows.",
    capabilities: ["line-dasharray", "line-gradient", "line-progress", "arrows"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "vector-feature-state",
    name: "Vector Feature State",
    description:
      "Hover/select vector-tile buildings via feature-state on the base style's composite source.",
    capabilities: [
      "vector tiles",
      "composite source",
      "setFeatureState",
      "hover",
      "selection",
      "fill-extrusion",
    ],
    category: "layers",
    subcategory: "vector-tiles",
    tags: ["interactive", "vector", "3d"],
    provider: "mapbox",
  },
  {
    patternId: "vector-road-styling",
    name: "Vector Road Styling",
    description:
      "Restyle vector-tile roads using match + interpolate expressions (composite/road).",
    capabilities: [
      "vector tiles",
      "composite source",
      "match",
      "interpolate",
      "filters",
    ],
    category: "layers",
    subcategory: "vector-tiles",
    tags: ["vector"],
    provider: "mapbox",
  },
  {
    patternId: "vector-debug-tools",
    name: "Vector Debug Tools",
    description:
      "Toggle tile boundaries, collision boxes, overdraw inspector, and other debug overlays.",
    capabilities: ["debug", "tile boundaries", "collision boxes", "overdraw"],
    category: "layers",
    subcategory: "vector-tiles",
    tags: ["vector"],
    provider: "mapbox",
  },
  {
    patternId: "3d-buildings",
    name: "3D Buildings",
    description: "Extruded 3D building footprints with height and color.",
    capabilities: ["fill-extrusion", "pitch", "lighting"],
    category: "layers",
    subcategory: "raster-3d",
    tags: ["3d", "interactive", "vector"],
    provider: "mapbox",
  },
  {
    patternId: "terrain-hillshade",
    name: "Terrain + Hillshade",
    description:
      "3D terrain with hillshade relief and optional atmospheric sky.",
    capabilities: ["raster-dem", "terrain", "hillshade", "sky"],
    category: "layers",
    subcategory: "raster-3d",
    tags: ["3d", "raster"],
    provider: "mapbox",
  },
  {
    patternId: "nasa-gibs-true-color",
    name: "NASA GIBS True Color",
    description:
      "Daily MODIS global satellite imagery via NASA GIBS WMTS raster tiles.",
    capabilities: [
      "WMTS raster",
      "MODIS Terra/Aqua",
      "daily imagery",
      "opacity",
    ],
    category: "layers",
    subcategory: "raster-3d",
    tags: ["raster", "api-required"],
    provider: "mapbox",
  },
  {
    patternId: "raster-overlay",
    name: "Raster Overlay (XYZ/WMTS)",
    description:
      "Add/remove a generic raster tile overlay using a URL template (XYZ/WMTS), with live opacity and attribution.",
    capabilities: [
      "raster source",
      "tile URL template",
      "min/max zoom",
      "opacity",
      "attribution",
    ],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive", "raster"],
    provider: "mapbox",
  },
  {
    patternId: "image-overlay",
    name: "Image Overlay (Bounds)",
    description:
      "Render an image overlay positioned by geographic bounds, with opacity and fit-to-bounds controls.",
    capabilities: ["image source", "bounds", "opacity", "fit bounds", "toggle"],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "geojson-overlay",
    name: "GeoJSON Overlay",
    description: "Paste/upload GeoJSON and render it with styling controls.",
    capabilities: [
      "GeoJSON source",
      "import",
      "export KML",
      "export GPX",
      "fit bounds",
      "styling",
    ],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "feature-state",
    name: "Feature State",
    description:
      "Hover + click selection via setFeatureState (no data mutation).",
    capabilities: ["setFeatureState", "hover", "selection"],
    category: "layers",
    subcategory: "data-import",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "rendalen-data",
    name: "Rendalen Data",
    description:
      "Load and display GeoJSON data from the Rendalen municipality dataset.",
    capabilities: ["GeoJSON", "data loading", "feature display"],
    category: "layers",
    subcategory: "data-import",
    tags: ["geojson", "api-required"],
    provider: "mapbox",
  },
  {
    patternId: "heatmap",
    name: "Heatmap",
    description: "Density-based heatmap visualization of point data.",
    capabilities: ["heatmap layer", "weight", "radius", "color ramp"],
    category: "data-viz",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "choropleth",
    name: "Choropleth",
    description: "Thematic map with areas colored by data values.",
    capabilities: ["data-driven styling", "legend", "interpolation"],
    category: "data-viz",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "export-image-print",
    name: "Screenshot / Export Image + Print View",
    description:
      "Export the current map view as PNG with attribution, and open a print-friendly layout (dual-engine Mapbox/MapLibre).",
    capabilities: ["canvas export", "attribution overlay", "print CSS", "preserveDrawingBuffer"],
    category: "data-viz",
    tags: ["interactive"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "time-slider",
    name: "Time Slider Playback",
    description:
      "Scrub and play through time-series point events with cumulative filtering (dual-engine Mapbox/MapLibre).",
    capabilities: ["setFilter", "time series", "playback", "circle layer", "range slider"],
    category: "data-viz",
    tags: ["interactive", "animation"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "clustered-markers",
    name: "Clustered Markers",
    description: "Cluster nearby points and expand on zoom.",
    capabilities: ["clustering", "cluster count", "zoom expand"],
    category: "markers",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "custom-popups",
    name: "Custom Popups",
    description: "Interactive popups with custom HTML content on markers.",
    capabilities: ["popups", "custom HTML", "event handling"],
    category: "markers",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "pulsing-dot",
    name: "Pulsing Dot",
    description:
      "Animated marker icon using StyleImageInterface + triggerRepaint.",
    capabilities: ["addImage", "StyleImageInterface", "animation"],
    category: "markers",
    tags: ["animation"],
    provider: "mapbox",
  },
  {
    patternId: "route-display",
    name: "Route Display",
    description: "Display driving/walking routes between waypoints.",
    capabilities: ["Directions API", "route line", "waypoints"],
    category: "navigation",
    subcategory: "routing",
    tags: ["api-required", "interactive"],
    provider: "mapbox",
  },
  {
    patternId: "animated-route",
    name: "Animated Route",
    description: "Animate a moving highlight along a route line.",
    capabilities: ["line-gradient", "line-progress", "animation loop"],
    category: "navigation",
    subcategory: "routing",
    tags: ["animation", "api-required"],
    provider: "mapbox",
  },
  {
    patternId: "distance-measurement",
    name: "Distance Measurement",
    description: "Click-to-measure distances between points on the map.",
    capabilities: [
      "turf.js",
      "line distance",
      "export GeoJSON",
      "export KML",
      "export GPX",
      "interactive",
    ],
    category: "navigation",
    subcategory: "measurement",
    tags: ["interactive", "turf", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "area-measurement",
    name: "Area Measurement",
    description: "Draw a polygon and calculate area with unit conversions.",
    capabilities: [
      "turf.js",
      "polygon area",
      "export GeoJSON",
      "export KML",
      "export GPX",
      "draw interaction",
    ],
    category: "navigation",
    subcategory: "measurement",
    tags: ["interactive", "turf", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "geolocation",
    name: "Geolocation",
    description: "Show your current position with an accuracy circle.",
    capabilities: ["navigator.geolocation", "accuracy circle", "follow"],
    category: "navigation",
    subcategory: "location-analysis",
    tags: ["interactive"],
    provider: "mapbox",
  },
  {
    patternId: "geocoding-search",
    name: "Geocoding Search",
    description:
      "Search for places using Mapbox, Nominatim, or Photon. Fly to results and optionally drop a pin.",
    capabilities: [
      "geocoding",
      "provider switch",
      "debounce + abort",
      "flyTo",
      "pin overlay",
    ],
    category: "navigation",
    subcategory: "location-analysis",
    tags: ["api-required", "interactive"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "reverse-geocoding",
    name: "Reverse Geocoding On Click",
    description:
      "Click the map to resolve the nearest address/place with fallback (Mapbox/Nominatim), plus copy-to-clipboard.",
    capabilities: ["reverse geocoding", "click interaction", "fallback", "clipboard"],
    category: "navigation",
    subcategory: "location-analysis",
    tags: ["api-required", "interactive"],
    // Catalog/provider is Mapbox for compatibility with existing loader/test expectations.
    // The demo itself supports MapLibre via an engine toggle inside the pattern view.
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "overpass-poi-overlay",
    name: "Overpass POI Overlay",
    description:
      "Query OpenStreetMap features via Overpass API and render nearby POIs. Click map to set the search center.",
    capabilities: ["Overpass API", "OSM POIs", "GeoJSON", "no-key"],
    category: "navigation",
    subcategory: "location-analysis",
    tags: ["interactive", "open-source", "no-key", "api-required", "geojson"],
    provider: "mapbox",
  },
  {
    patternId: "isochrones",
    name: "Isochrones (Travel Time)",
    description:
      "Visualize reachable areas within specific travel time intervals using Valhalla.",
    capabilities: ["isochrones", "Valhalla", "travel time", "multi-profile"],
    category: "navigation",
    subcategory: "location-analysis",
    tags: ["api-required", "interactive"],
    provider: "mapbox",
  },
  {
    patternId: "keyboard-shortcuts",
    name: "Keyboard Shortcuts + Presets",
    description: "Keyboard shortcuts for camera presets and UI toggles.",
    capabilities: ["flyTo", "keyboard events", "camera presets", "help panel"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "box-select",
    name: "Box Select (Shift-Drag)",
    description: "Shift-drag rectangle selection with feature-state highlighting.",
    capabilities: ["queryRenderedFeatures", "feature-state", "shift-drag", "bbox selection"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "draggable-points",
    name: "Draggable Points",
    description: "Drag point features and update underlying GeoJSON in real time.",
    capabilities: ["dragPan", "mousemove", "setData", "cursor states"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "context-menu",
    name: "Right-Click Context Menu",
    description: "Context menu with copy coordinates, drop pin, and clear pins.",
    capabilities: ["contextmenu", "clipboard", "GeoJSON pins", "positioned overlay"],
    category: "layers",
    subcategory: "fundamentals",
    tags: ["interactive"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "hexbin-grid",
    name: "Hexbin / Grid Aggregation",
    description: "Aggregate points into hexagonal bins styled by count using Turf.js.",
    capabilities: ["hexGrid", "pointsWithinPolygon", "data-driven fill", "legend"],
    category: "data-viz",
    tags: ["interactive", "turf", "geojson"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
  {
    patternId: "cluster-spiderfy",
    name: "Cluster Spiderfy",
    description: "Expand dense clusters into a radial spider layout for selection.",
    capabilities: ["clustering", "getClusterLeaves", "spider layout", "leg lines"],
    category: "markers",
    tags: ["interactive", "geojson"],
    provider: "mapbox",
    workbenchCompatible: false,
  },
];

export const MAPLIBRE_ENTRY_DATA: CatalogEntry = {
  patternId: "maplibre",
  name: "MapLibre GL",
  description: "Open-source map rendering with OpenFreeMap vector basemaps.",
  capabilities: [
    "Vector tiles",
    "OpenFreeMap",
    "No-key basemap",
    "open source",
  ],
  category: "layers",
  tags: ["open-source", "no-key", "vector"],
  provider: "maplibre",
};
